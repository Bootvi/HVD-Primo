function buildSKCgallery() {
	$(".EXLDetailsContent li[id^='lds41']").parents(".EXLDetailsContent").each(function() {
		...
		// 20160715 CB wrapping everything in an if to test whether this is an Aelph record
		// See salesforce case 00161769, PCI is using our local fields
		if ($(this).children("ul").children("li[id^=Permalink]").length > 0) {
			
			console.log($(this).children("ul").children("li[id^=Permalink]"));
